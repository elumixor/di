// biome-ignore-all lint/suspicious/noExplicitAny: It makes sense here
...
const serviceMap = target.__di_injectable_map__ as Map<unknown, object>;
...
function injectable<TBase extends AnyConstructor>(Base: TBase): TBase {
  ...
  class Wrapper extends Base {
    constructor(...args: ConstructorParameters<TBase>) {
      if (isProvided(Wrapper)) throw new Error("Trying to provide a different child for the same provided parent");
